...
for(j in 1:2)
{
	...
	lm_ìnteraction 	<- lm(expression~clone+day,data=D)
	lm_time 		<- lm(expression~clone,data=D)
	lm_genotype 	<- lm(expression~day,data=D)
	summ <- summary(lm_full)
	waldtest(lm_full,lm_ìnteraction)		# significance of interaction
	waldtest(lm_ìnteraction,lm_time)		# significance of time; ignoring interaction
	waldtest(lm_ìnteraction,lm_genotype)	# significance of genotype; ignoring interaction
